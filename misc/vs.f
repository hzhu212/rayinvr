...
c     |             **************  V S  **************              |
c     |                                                              |
c     |         Convert a P-wave velocity model to an S-wave         |
c     |            model using a constant Poisson's ratio            |
c     |                                                              |
...
c                 
c        12 -- output: S-wave velocity model
c                 
c
c     ----------------------------------------------------------------
c                 
...
c
      write(6,6)
6     format(/"Enter Poisson's ratio")
      read(5,*) pois
c
      nrzmax=ppcntr/10
      nrvmax=ppvel/10
...
c
      do 1030 i=1,nlayer
         do 1040 j=1,nvel(i,1)
            vf(i,j,1)=sqrt((1.-2.*pois)/(2.*(1.-pois)))*vf(i,j,1)
1040     continue
         do 1050 j=1,nvel(i,2)
            vf(i,j,2)=sqrt((1.-2.*pois)/(2.*(1.-pois)))*vf(i,j,2)
1050     continue
1030  continue
c
c     write out the S-wave velocity model
c
      do 570 i=1,nlayer
         nstart=1  
  590    j1=nstart
