...
c     |             ***********  V D I F F  ***********              |
c     |                                                              |
c     |         Take the difference of two velocity models           |
c     |               with the same parameterization                 |
c     |                                                              |
...
c     I/O units:  
c                 
c        11 -- input:  first velocity model
c                 
c        12 -- input:  second velocity model
c                 
c        13 -- output: difference (second - first)
c                 
c
c     ----------------------------------------------------------------
c                 
...
c
      write(6,1)
1     format(/'Enter file name of model #1')
      read(5,4) file1
      write(6,2)
2     format(/'Enter file name of model #2')
      read(5,4) file2
4     format(a80)
c                 
      open(unit=11, file=file1, status='old')
      open(unit=12, file=file2, status='old')
      open(unit=13, file='vdiff.out')
c
      write(6,5)
...
242   continue
c
c     take the difference of the two models
c
      zmax=0.
      do 1010 i=1,ncont1
         do 1020 j=1,nzed1(i)
            if(xm1(i,j).ne.xm2(i,j)) go to 998
            zm1(i,j)=zm2(i,j)-zm1(i,j)
            if(abs(zm1(i,j)).gt.zmax) zmax=abs(zm1(i,j))
            ivarz(i,j)=0
1020     continue
1010  continue
c
      ...
      do 1030 i=1,nlayer1
         do 1040 j=1,nvel1(i,1)
            ...
            vf1(i,j,1)=vf2(i,j,1)-vf1(i,j,1)
            if(abs(vf1(i,j,1)).gt.vmax) vmax=abs(vf1(i,j,1))
            ivarv(i,j,1)=0
1040     continue
         do 1050 j=1,nvel1(i,2)
            if(xvel1(i,j,2).ne.xvel2(i,j,2)) go to 998
            vf1(i,j,2)=vf2(i,j,2)-vf1(i,j,2)
            if(abs(vf1(i,j,2)).gt.vmax) vmax=abs(vf1(i,j,2))
            ivarv(i,j,2)=0
1050     continue
1030  continue
c
      write(6,105) zmax,vmax
105   format(/'maximum depth difference (km):     ',f10.3/
     +        'maximum velocity difference (km/s):',f10.3/)
c
