...
c
      do 1270 i=1,nlayer
         do 1280 j=1,nzed(i)
            if(ivarz(i,j).gt.0) then
              np=np+1
              if(j.gt.1.and.j.lt.nzed(i)) then
                dist=(xm(i,j+1)-xm(i,j-1))/2.
                d(np,np+1)=-1./dist
                d(np,np)=2./dist
                d(np,np-1)=-1./dist
              endif
...
c
         if (nvel(i,1).gt.0) then
           do 1310 j=1,nvel(i,1)
             if(ivarv(i,j,1).gt.0) then
               np=np+1
               if(j.gt.1.and.j.lt.nvel(i,1)) then
                 dist=(xvel(i,j+1,1)-xvel(i,j-1,1))/2.
                 d(np,np+1)=-1./dist
                 d(np,np)=2./dist
                 d(np,np-1)=-1./dist
               end if
...
c
         if (nvel(i,2).gt.0) then
           do 1430 j=1,nvel(i,2)
             if(ivarv(i,j,2).gt.0) then
               np=np+1
               if(j.gt.1.and.j.lt.nvel(i,2)) then
                 dist=(xvel(i,j+1,2)-xvel(i,j-1,2))/2.
                 d(np,np+1)=-1./dist
                 d(np,np)=2./dist
                 d(np,np-1)=-1./dist
               end if
...
c
      if(ifrbnd.eq.1) then
        do 1880 i=1,nfrefl
          do 1890 j=1,npfref(i)
            if(ivarf(i,j).eq.1) then
              np=np+1
              if(j.gt.1.and.j.lt.npfref(i)) then
                dist=(xfrefl(i,j+1)-xfrefl(i,j-1))/2.
                d(np,np+1)=-1./dist
                d(np,np)=2./dist
                d(np,np-1)=-1./dist
              end if
