...
c                
     ...
     +     xfrefl(pfrefl,ppfref),zfrefl(pfrefl,ppfref),zmb(ppcntr)
...
3115        zmb(k)=zm(i,k)
         do 280 j=1,nzed(i)
            ...
            if(ivarz(i,j).gt.0) then
              nvarw=nvarw+1
              write(63,255) xm(i,j),-zm(i,j),hit(nvarw)/hnorm
              write(66,255) xm(i,j),-zm(i,j),1.3*hit(nvarw)/hnorm
              write(73,255) xm(i,j),-zm(i,j),
     +                      res(nvarw,nvarw)/rnorm
              write(76,255) xm(i,j),-zm(i,j),
     +                      1.3*res(nvarw,nvarw)/rnorm
              write(83,255) xm(i,j),-zm(i,j),
     +                      abs(res(nvarw,nker))/rnorm
              write(86,255) xm(i,j),-zm(i,j),
     +                      abs(1.3*res(nvarw,nker))/rnorm
255           format(3f10.3)
           ...
           do 310 j=1,nvel(i,1)
              if(ivarv(i,j,1).gt.0) then
                ...
                if(nzed(i).eq.1) then
                  ...
                  do 3111 k=1,nzed(i)-1
                     if(xvel(i,j,1).ge.xm(i,k).and.
     +                  xvel(i,j,1).le.xm(i,k+1)) then
                       zhit=(zmb(k+1)-zmb(k))/(xm(i,k+1)-xm(i,k))*
     +                      (xvel(i,j,1)-xm(i,k))+zmb(k)
                       go to 3113
                     end if
3111              continue
                end if
3113            write(64,265) xvel(i,j,1),-zhit-zshift,hit(nvarw)
                write(74,255) xvel(i,j,1),-zhit-zshift,
     +                        res(nvarw,nvarw)
                write(84,255) xvel(i,j,1),-zhit-zshift,
     +                        abs(res(nvarw,nker))
265             format(2f10.3,i10)
           ...
           do 430 j=1,nvel(i,2)
              if(ivarv(i,j,2).gt.0) then
                ...
                if(nzed(i+1).eq.1) then
                  ...
                  do 3112 k=1,nzed(i+1)-1
                     if(xvel(i,j,2).ge.xm(i+1,k).and.
     +                  xvel(i,j,2).le.xm(i+1,k+1)) then
                       zhit=(zm(i+1,k+1)-zm(i+1,k))/
     +                      (xm(i+1,k+1)-xm(i+1,k))*
     +                      (xvel(i,j,2)-xm(i+1,k))+zm(i+1,k)
                       go to 3114
                     end if
3112              continue
                end if
3114            write(64,265) xvel(i,j,2),-zhit+zshift,hit(nvarw)
                write(74,255) xvel(i,j,2),-zhit+zshift,
     +                        res(nvarw,nvarw)
                write(84,255) xvel(i,j,2),-zhit+zshift,
     +                        abs(res(nvarw,nker))
        ...
        do 880 j=1,nfrefl
           do 890 i=1,npfref(j)
              if(ivarf(j,i).eq.1) then
                nvarw=nvarw+1
                write(65,255) xfrefl(j,i),-zfrefl(j,i),hit(nvarw)/hnorm
                write(75,255) xfrefl(j,i),-zfrefl(j,i),
     +                        res(nvarw,nvarw)/rnorm
                write(85,255) xfrefl(j,i),-zfrefl(j,i),
     +                        abs(res(nvarw,nker)/rnorm)
