...
c                
     ...
     +     xm(pncntr,ppcntr),zm(pncntr,ppcntr),grad(player*ppvel),
     ...
     +     d(pnvar,pnvar),dtd(pnvar,pnvar),dtdm(pnvar),
     +     parbsf(pnvar),velw(9),bndw(9)
     ...
     +        ivarf(pfrefl,ppfref),npfref(pfrefl),parw(pnvar),
     +        iparz(player,ppcntr),iparv(player,ppvel,2)
c                 
...
115   format(2i5)
      read(11,1)
      do 6 i=1,nvar
         read(11,5) partyp(i),parorg(i),parw(i)
5        format(i5,f15.5,i10)
6     continue
      read(11,1)
      do 10 i=1,narinv
...
c
c change depths to layer thicknesses if required
c
      np=0
      if (ilayer.eq.1) then
        do 1281 i=2,nlayer
          do 1282 j=1,nzed(i)
            if(ivarz(i,j).gt.0) then
              np=np+1
              do 1284 k=1,nzed(i-1)
                if(abs(xm(i,j)-xm(i-1,k)).lt..005) then
                  parbsf(np)=zm(i,j)-zm(i-1,k)
                  goto 1282
                end if
1284          continue
            endif
...
c
c calculate difference matrix D
c
      do 1250 i=1,nvar
         do 1260 j=1,nvar
            d(i,j)=0.
...
c
         if (nvel(i,1).gt.0) then
           do 1310 j=1,nvel(i,1)
             if(ivarv(i,j,1).gt.0) then
               np=np+1
               if(j.gt.1) then
                 dist=xvel(i,j,1)-xvel(i,j-1,1)
                 d(np,np)=1./dist
                 d(np,np-1)=-1./dist
               end if
             end if
...
c
         if (nvel(i,2).gt.0) then
           do 1430 j=1,nvel(i,2)
             if(ivarv(i,j,2).gt.0) then
               np=np+1
               if(j.gt.1) then
                 dist=xvel(i,j,2)-xvel(i,j-1,2)
                 d(np,np)=1./dist
                 d(np,np-1)=-1./dist
               end if
             end if
...
c
      if(ifrbnd.eq.1) then
        do 1880 i=1,nfrefl
          do 1890 j=1,npfref(j)
            if(ivarf(i,j).eq.1) then
              np=np+1
              if(j.gt.1) then
                dist=xfrefl(i,j)-xfrefl(i,j-1)
                d(np,np)=1./dist
                d(np,np-1)=-1./dist
              end if
            end if
...
50    continue
c
c calculate DTD and (ATA+DTD)
c
      do 1520 i=1,nvar
         do 1530 j=1,nvar
           dtd(i,j)=0.
           do 1540 k=1,nvar
             dtd(i,j)=dtd(i,j)+d(k,i)*d(k,j)*dmpfct/parunc(k)
1540       continue
	   atad(i,j)=ata(i,j)+dtd(i,j)
1530     continue
1520  continue
c
c calculate DTDm
c
      if (ilayer.eq.1) then
        do 1550 i=1,nvar
          dtdm(i)=0.
          do 1560 j=1,nvar
             dtdm(i)=dtdm(i)+dtd(i,j)*parbsf(j)
1560      continue
1550    continue
      else
        do 1551 i=1,nvar
          dtdm(i)=0.
          do 1561 j=1,nvar
             dtdm(i)=dtdm(i)+dtd(i,j)*parorg(j)
1561      continue
1551    continue
      endif
c
c invert matrix (ATA+DTD)
c
      call matinv(atad,atadi,nvar)
c
c calculate parameter adjustments
c
      do 90 i=1,nvar
         dx(i)=0.
         do 100 j=1,nvar
            dx(i)=dx(i)+atadi(i,j)*(att(j)-dtdm(j))
100      continue
90    continue
c
c calculate resolution matrix
c
      do 110 i=1,nvar
         do 120 j=1,nvar
            res(i,j)=0.
...
c
      write(12,25) dmpfct
25    format(/'overall damping factor: ',f10.2)
      write(12,35)
...
c
c calculate layer thickness if held constant
c
      nlyr=0
      do 470 i=2,ncont
         do 480 j=1,nzed(i)
