...
c     |          **************  V D E P T H  **************         |
c     |                                                              |
c     |    Apply a bulk shift to the x-coordinates of a v.in file    |
c     |                                                              |
...
c
      write(6,6)
6     format(/'Enter bulk shift to apply to x-coordinates (km)')
      read(5,*) xshift
c
      nrzmax=ppcntr/10
      nrvmax=ppvel/10
...
c
      do 1031 i=1,nlayer
         do 1041 j=1,nvel(i,1)
            xvel(i,j,1)=xvel(i,j,1)+xshift
1041     continue
         do 1042 j=1,nvel(i,2)
            xvel(i,j,2)=xvel(i,j,2)+xshift
1042     continue
1031  continue
c
      do 1030 i=1,nlayer+1
         do 1040 j=1,nzed(i)
            xm(i,j)=xm(i,j)+xshift
1040     continue
1030  continue
c
